...
class swift():
	...
	def __init__(self):
		...
		self.setpoint = [[0,0,13],[7,-7,23],[0,-7,23],[-7,-7,23],[-7,0,23],[-7,7,23],[0,7,23],[7,7,23],[0,0,17]] # whycon marker at the position of the dummy given in the scene. Make the whycon marker associated with position_to_hold dummy renderable and make changes accordingly
		
		self.next_junction=0
		...
		self.organism_type = "unknown"
		self.previous_organism_type="none"
		self.dis=0
...
#		
	...
	def kis_corner_me_mila(self):
		...
		if self.centroid_led_x > 333.33 and self.centroid_led_y> 333.33 :
			...
			if self.detecting_point==2 or self.detecting_point==3:
	...
	def check_centroid(self):
		...
		if (self.centroid_led_x<350 and self.centroid_led_x>150 and self.centroid_led_y>150 and self.centroid_led_y<350 and self.publish_count<3) and self.drone_position[2]>24:
			print("target locked and publish")
			self.organism()
			
			print(self.organism_type)
			if self.organism_type != self.previous_organism_type :
				self.previous_organism_type=self.organism_type
	...
	def pid(self):
		...
		if (self.led_detection==1 and self.detecting_point<10 and all(self.drone_position[i] <self.max_err[i] for i in range(3)) and all(self.drone_position[i]> self.min_err[i] for i in range(3)) and self.publish_count<3) :
	...
	#		self.target[2] = (37 - self.led_detecting_point[2])/3
		...
		if (self.publish_count==3 and all(self.drone_position[i] <self.max_err[i] for i in range(3)) and all(self.drone_position[i]> self.min_err[i] for i in range(3))) :
			print("final")
			self.target= [11,11,37]
		if(self.drone_position[0]>10 and self.drone_position[1]>10 and self.publish_count==3 ):
	...
	#	8. Add error_sum
		...
		if all(abs(error[i]) < 0.2 for i in range(3)) and self.publish_count==3:
